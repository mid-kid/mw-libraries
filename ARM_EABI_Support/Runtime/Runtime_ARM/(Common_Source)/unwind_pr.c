...
static  uint32_t core_get(_Unwind_Context *context, uint32_t regno)
{
  uint32_t val;
...
static  void core_set(_Unwind_Context *context, uint32_t regno, uint32_t newval)
{
  /* This call is required to never fail if given a valid regno */
  _Unwind_VRS_Set(context, _UVRSC_CORE, regno, _UVRSD_UINT32, &newval);
}

static  uint32_t count_to_mask(uint32_t count) {
  return (1 << count) - 1;
}
                                              ...
                                              uint32_t idx)
{
...
#ifdef CW_DIAGNOSTICS
		printf("Return address = 0x%x",rtn_addr);
#endif
  ...
  while (1) {
    ...
    if (ub == 0xc8) { /* 11001000: was pop fpa, now spare */
      DEBUGGER_BOTTLENECK(ucbp, _UASUBSYS_CPP, _UAACT_ENDING, _UAARG_ENDING_CPP_BADOPCODE);
      return _URC_FAILURE;
    }
    if (ub == 0xc9) { /* 11001001 sssscccc: pop VFP from FSTMFDD */
      uint32_t discriminator = next_unwind_byte(&ud);
      discriminator = ((discriminator & 0xf0) << 12) | ((discriminator & 0x0f) + 1);
