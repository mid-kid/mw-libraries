...
/*
 * RCS $Revision: 1.1 $
 * Checkin $Date: 2007/03/02 06:18:06 $
 ...
 */
void _Unwind_Resume(UCB *);
void _Unwind_Complete(UCB *);
...
extern "C" {
  ...
  void __cxa_throw(void *, const type_info *, cppdtorptr);
...
extern "C"  void __ARM_exceptions_buffer_required(void);
                                         ...
                                         bool is_reference_type, void **matched_objectpp)
{
  ...
  current_objectp = original_objectp;
...
/* Redeclare _Unwind_RaiseException as weak (if  is defined
 * appropriately) so the use from __cxa_rethrow does not on its own
 * force the unwind library to be loaded.
 */

extern "C"  _Unwind_Reason_Code _Unwind_RaiseException(UCB *ucbp);
...
extern UCB * __ARM_cxa_end_cleanup(void);
extern void _Unwind_Resume(UCB *);
...
void __ARM::eh_catch_semantics(UCB *ucbp)
{
  __cxa_eh_globals *g = __cxa_get_globals();
  __cxa_exception *ep;
