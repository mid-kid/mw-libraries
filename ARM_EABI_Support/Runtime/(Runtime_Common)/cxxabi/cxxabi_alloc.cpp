...
namespace {
	void DestroyArrayRange(unsigned char *first,unsigned char *last,_CXXABI_SIZE_T element_size,__cxa_cdtor_type destructor)
	{
		try {
			while(last>first)
			{
	...
	__cxa_cdtor_type destructor)
{
	...
	if(constructor)
	{
		...
		else
		{
			...
			_MSL_CXA_VEC_CTOR_RETURN (array_address);
}
...
{	
	...
	if(constructor)
	{
		...
		_MSL_CXA_VEC_CTOR_RETURN (dest_array);
