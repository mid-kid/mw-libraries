...
//				we catch this later
				if (exp_value > DBL_MAX_10_EXP)
					*overflow = 1;
//				if (exp_value > 10000000)
//					exp_value = 10000000;  // prevent long overfow
				c = fetch();
				break;
...
#if 0	
	if (overflow || (/*value != 0.0*/  (abs_value < DBL_MIN || abs_value > DBL_MAX))) /* cc P70429.c Execution passed */
#else
	if (overflow)
#endif
		errno = ERANGE;
 ...
 * cc  040625 Added unaligned accesses bug fix
 */
