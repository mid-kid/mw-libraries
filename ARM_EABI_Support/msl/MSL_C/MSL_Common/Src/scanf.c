					...
					else
					{
							...
							((rval = ((*ReadProc)(ReadProcArg, 0, __GetAChar))) != EOF))	/*- mm 990325 -*/ /*- mm 010426 -*/ /*- mm 011207 -*/
						{
							...
							if (format.argument_options == wchar_argument)		/*- mm 990407 -*/
							{
								...
								arg_ptr += sizeof(wchar_t); /*-  was ((wchar_t*)arg_ptr)++;  -*/
							}
							else
...
int _MSL_CDECL vfscanf(FILE * _MSL_RESTRICT file, const char * _MSL_RESTRICT format, va_list arg)
{
	int retval;
	int crtrgn;
	if (file == stdin)
