	 ...
	 long limit, long *nameSizeParam, long functOffset, long diskOffset)
{
	...
	NameOffsetEntryPtr	currentOffsetPtr;
	long				nameOffset = 0L;
	WriteBufferType		buffer;
    
	currentOffsetPtr = nameOffsetPtr = (NameOffsetEntryPtr)globals->nameOffsetPtr;

	if ( noErr == InitializeWriteBuffer(&buffer, refNum,0))
	{
			...
			//	i = 1; /* collect detailed - skip the first element, fake main */	
		        ...
		        ///****** write data section 		
			for (i = globals->header.method == collectSummary?0:1; error == noErr && i < limit; i++)
			{
				...
				if (globals->header.method == collectDetailed)
				{
						if((Shared = FindInOffsetTable(globals, nameOffsetPtr, fp->name, nameOffset)) != -1L)
					{
				...
				else // write name and offset  for wrinting name section later
				{
					fInfo.nameOffset  = currentOffsetPtr->offset = nameOffset;
					currentOffsetPtr->name = fp->name;
					currentOffsetPtr++;
				}
				...
				if (noErr == WriteToBuffer(&buffer,&fInfo,sizeof(fInfo)) && Shared == -1L)
					nameOffset += my_strlen(fp->name);
			}
			error  = FlushWriteBuffer(&buffer, true);
			
			// ******* write names section
			if (error == noErr)
			{
			   	if (globals->header.method == collectDetailed) // need sort ... otherwise already sorted
			             qsort((void*)globals->nameOffsetPtr,(long)globals->nameOffsetSize,sizeof(NameOffsetEntry),&compare_offset);
			     
			     for(int i = 0; i < globals->nameOffsetSize; i++ )
			          if(nameOffsetPtr[i].name != 0)
			           if(noErr != WriteToBuffer(&buffer,nameOffsetPtr[i].name,strlen(nameOffsetPtr[i].name)+1))
			             break;
