...
return(__msl_mod(year, 4) == 0 && 
				(__msl_mod(year, 100) != 0 || __msl_mod(year, 400) == 100));
}
...
char * _MSL_CDECL asctime(const struct tm * tm)
{
	...
	if (tm)
		...
		__msl_asciitime(err_tm, _MSL_LOCALDATA(asctime_result));

	return(_MSL_LOCALDATA(asctime_result));
}
...
struct tm * _MSL_CDECL gmtime(const time_t * timer)
{
	...
	if (!timer)
	{
		__msl_clear_tm(&_MSL_LOCALDATA(gmtime_tm));
		return(&_MSL_LOCALDATA(gmtime_tm));
	}
	...
	__msl_time2tm(time, &_MSL_LOCALDATA(gmtime_tm));
	_MSL_LOCALDATA(gmtime_tm).tm_isdst = 0;
...
struct tm * _MSL_CDECL localtime(const time_t * timer)
{
	...
	if (!timer)
		...
		__msl_time2tm(*timer, &_MSL_LOCALDATA(localtime_tm));
...
#if defined(__PPC_EABI__) || defined(__MIPS__) /*__dest_os == __mips_bare */ 			/*- scm 970709 -*/
