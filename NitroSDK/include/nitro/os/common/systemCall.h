/*---------------------------------------------------------------------------*
  ...
  $Log: systemCall.h,v $
  Revision 1.23  01/18/2006 02:12:28  kitase_hirotake
  do-indent

  Revision 1.22  11/21/2005 02:25:10  terui
  Reconstruct SVC_WaitVBlankIntr definition.

  Revision 1.21  10/24/2005 00:49:42  adachi_hiroaki
  Removed SDK_WEAK_SYMBOL specification from SVC_WaitVBlankIntr()

  Revision 1.20  10/21/2005 09:00:02  yada
  let SVC_WaitVBlankIntr be weak symbol.

  Revision 1.19  03/01/2005 01:57:00  yosizaki
  Revised copyright year

  Revision 1.18  02/28/2005 05:26:01  yosizaki
  do-indent.

  Revision 1.17  11/02/2004 09:48:48  yada
  just fix comment

  Revision 1.16  07/20/2004 07:27:11  yada
  system call functions were renewed

  Revision 1.15  05/26/2004 11:10:02  terui
  change comment of SVC_WaitByLoop

  Revision 1.14  04/15/2004 11:52:27  yada
  delete SVC_ReadCard

  Revision 1.13  04/07/2004 02:03:17  yada
  fix header comment

  Revision 1.12  2004/03/26 13:11:31  yasu
  #if SDK_ARM7 -> #ifdef SDK_ARM7

  Revision 1.11  03/25/2004 09:43:20  yada
  Added SVC_Halt(), SVC_WaitByLoop() for ARM7

  Revision 1.10  2004/03/17 10:36:50  yasu
  fix SVC_CpuCopyFast

  Revision 1.9  02/10/2004 11:34:05  yada
  Delete SystemCall().

  Revision 1.8  02/05/2004 07:27:19  yada
  historic IRIS character string returned from NITRO to IRIS.

  Revision 1.7  2004/02/05 07:09:03  yasu
  change SDK prefix iris -> nitro

  Revision 1.6  02/05/2004 00:19:53  yada
  With #define made so that anything enclosed by only { } will become do{ }while(0).

  Revision 1.5  01/19/2004 06:14:09  yada
  ...
  Revision 1.3  11/28/2003 01:54:54  yada
  Reflects REDSDK 03-11-27

  Revision 1.2  11/11/2003 12:27:28  yada
  Moved some system call portions from interrupt.h to systemCall.h

  Revision 1.1  11/07/2003 07:55:16  yada
...
/*---------------------------------------------------------------------------*
  ...
  Description:  wait for VBlank
              ...
              - When using multiple interrupts, you can reduce the system  
                call overhead compared to when SVC_Halt() is called
                repeatedly.
              Entering halt state while using thread also stops other threads  
...
/*---------------------------------------------------------------------------*
  ...
  Description:  loop specified times
              ...
              - Performs count loop process specified in system ROM.  
              - It takes four cycles for one loop.
              - In the state where normal subprocessor has priority over main memory,  
                you can reduce stalling in the main processor by operating  
                subprocessor on system ROM.  
              - When it is necessary to give priority to main processor in main memory display mode,
                there may be a situation where calling on the main processor side is effective.  
                But if most periods are operated
                in cache and TCM, it is not necessary to call from main processor.  
  ...
  Description:  unpack bits
              ...
              * Align the destination address to a 4-byte boundary.
              ...
              MIUnpackBitsParam Structure
                    ...
                    u8  srcBitNum           1 Source data. Number of bits. 
                    u8  destBitNum          1 Destination data. Number of bits.  
  ...
  Description:  uncompress LZ77
              ...
              * Expands LZ77 compressed data, and writes in 8-bit units.
              ...
              - If the size of the compressed data is not a multiple of 4,  
                where possible adjust by packing with 0.  
...
/*---------------------------------------------------------------------------*
  ...
  Description:  quotient of division
              ...
              - The register value is restored with r0=numer/denom, r1=number%denom,
                r3=|numer/denom|.  
...
/*---------------------------------------------------------------------------*
  ...
  Description:  square root
              ...
              - To improve accuracy, make the argument a multiple of 2 only, left shift, and pass.
                Also shift return value and perform arranged in columns.  
              - In order to reduce code size, this is not very fast.  
...
/*---------------------------------------------------------------------------*
  ...
  Description:  halt
                ...
                - When the CPSR IRQ disable flag is set (OS_DisableInterrupts) ,   
                  returns from halt, but   
                  an interrupt is not generated.  
                - Cannot return when IME halts  (OS_DisableIrq)   
                  in cleared state. 
...
/*---------------------------------------------------------------------------*
  ...
  Description:  sleep
              ...
              - Suspends the power.  
              - When RTC, key, card, cartridge, or console open interrupt is permitted,
                (set in IE) , returns by generating the corresponding interrupt
                request condition.  
              - Power is suspended so IF flag is set return immediately  after.  
                But when the  interrupt request signal to the terminal to the terminal is in there until the CPU restarts,
                the IF flag is set at that time.
...
/*---------------------------------------------------------------------------*
  ...
  Description:  set sound bias
              ...
              Arguments:
                   stepLoops     loop number during sound bias change 1 step  (4 cycles/loop)  
                                 Sound bias is changed slowly with size of value. 
...
/*---------------------------------------------------------------------------*
  ...
  Description:  set sound bias
              ...
              - Moves sound BIAS from the middle value (0x200)  to 0.  
