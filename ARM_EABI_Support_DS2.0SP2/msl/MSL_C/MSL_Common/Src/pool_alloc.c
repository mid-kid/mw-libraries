 ...
 *
 *  The macro _MSL_POOL_ALIGNMENT may be defined to allow modifying
 *  the alignment without editing this file. The alignment must still
 *  be a multiple of 4, e.g.
 *  #define _MSL_POOL_ALIGNMENT 4L
 */

#ifndef _MSL_POOL_ALIGNMENT
	#if (defined(__POWERPC__) || defined(__INTEL__) || defined(__MIPS__))
	
		#define _MSL_POOL_ALIGNMENT 8L		/* 8-byte alignment */
	#else
	
		#define _MSL_POOL_ALIGNMENT 4L		/* 4-byte alignment */
	#endif
#endif /* _MSL_POOL_ALIGNMENT */

#define alignment (_MSL_POOL_ALIGNMENT - 1L)
...
#ifndef __ALTIVEC__
	if (size <= 0 || size > ULONG_MAX - (sizeof(tag_word) + alignment + heap_overhead))  
		return(0);
	
	size += sizeof(tag_word);
	size  = (size + alignment) & ~alignment;
#else
	if (size <= 0 || size > ULONG_MAX - (sizeof(tag_word) + pool_obj->options.block_alignment + heap_overhead))  
		return(0);
	size += sizeof(tag_word);
...
#endif /* !(defined(_MSL_CLASSIC_MALLOC) || defined(_MSL_PRO4_MALLOC)) */
 ...
 * cc  040629 Changed #ifdef __ALTIVEC__ to #ifndef __ALTIVEC
 */